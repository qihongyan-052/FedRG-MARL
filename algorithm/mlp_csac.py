...
class LocalMLPCSACAgent:
    def __init__(self, config: LocalMLPCSACConfig) -> None:
        self.config = config
        ...
        self.cost_critic = MLPCriticNetwork(state_dim=self.state_dim, action_dim=self.action_dim, hidden_dim=hidden_dim).to(self.device)
        ...
        self.cost_critic_optimizer = torch.optim.Adam(
            [
                {"params": self.cost_critic.q1.l1.parameters(), "lr": config.critic_backbone_lr},
                {"params": self.cost_critic.q1.l2.parameters(), "lr": config.critic_backbone_lr},
                {"params": self.cost_critic.q1.l3.parameters(), "lr": config.critic_head_lr},
                {"params": self.cost_critic.q2.l1.parameters(), "lr": config.critic_backbone_lr},
                {"params": self.cost_critic.q2.l2.parameters(), "lr": config.critic_backbone_lr},
                {"params": self.cost_critic.q2.l3.parameters(), "lr": config.critic_head_lr},
            ]
        )
    ...
    def evaluate_cmdp_score(self, park_graph: Dict[str, Any], node_action: torch.Tensor) -> float:
        with torch.inference_mode():
            ...
            qc1, qc2 = self.cost_critic(state_vec.unsqueeze(0), fixed_action.unsqueeze(0), valid_mask.unsqueeze(0))
    ...
    def update(self) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            qcf1_next_target, qcf2_next_target = self.cost_critic_target(next_states, next_action, next_valid_masks)
        ...
        qcf1, qcf2 = self.cost_critic(states, action_batch, valid_masks)
        qcf1_loss = F.mse_loss(qcf1, next_qc_value)
        qcf2_loss = F.mse_loss(qcf2, next_qc_value)
        cost_critic_loss = qcf1_loss + qcf2_loss
        ...
        qcf1_pi, qcf2_pi = self.cost_critic(states, pi, valid_masks)
        mean_qcf_pi = 0.5 * (qcf1_pi + qcf2_pi)
        cost_term = self.lambda_value.detach() * mean_qcf_pi
        actor_loss = ((self.alpha.detach() * log_pi) - min_qf_pi + cost_term).mean()
    ...
    def get_shared_state(self) -> Dict[str, Dict[str, torch.Tensor]]:
        ...
        if self.enable_federation and self.config.federate_critic_backbone:
            ...
            shared_state["cost_critic_backbone"] = {
                **{f"q1.l1.{k}": v.detach().cpu().clone() for k, v in self.cost_critic.q1.l1.state_dict().items()},
                **{f"q1.l2.{k}": v.detach().cpu().clone() for k, v in self.cost_critic.q1.l2.state_dict().items()},
                **{f"q2.l1.{k}": v.detach().cpu().clone() for k, v in self.cost_critic.q2.l1.state_dict().items()},
                **{f"q2.l2.{k}": v.detach().cpu().clone() for k, v in self.cost_critic.q2.l2.state_dict().items()},
            }
        return shared_state
    ...
    def load_shared_state(self, shared_state: Dict[str, Dict[str, torch.Tensor]]) -> None:
        ...
        if self.config.federate_critic_backbone and "critic_backbone" in shared_state:
            self._blend_critic_backbone(self.critic, self.critic_target, shared_state["critic_backbone"])
        if self.config.federate_critic_backbone and "cost_critic_backbone" in shared_state:
            self._blend_critic_backbone(self.cost_critic, self.cost_critic_target, shared_state["cost_critic_backbone"])
        self._sync_inference_actor()

    def _blend_critic_backbone(
        self,
        critic: MLPCriticNetwork,
        critic_target: MLPCriticNetwork,
        shared_block: Dict[str, torch.Tensor],
    ) -> None:
        for critic_name, critic_module, target_module in (
            ("q1", critic.q1, critic_target.q1),
            ("q2", critic.q2, critic_target.q2),
        ):
