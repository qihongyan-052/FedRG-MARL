...
@dataclass
class VerifyConfig:
    run_name: str = "rule_based_ev_charge_verify"
    seed: int = 3
...
def _build_rule_based_joint_action(obs: Dict[str, object]) -> Dict[str, object]:
    joint_action = {"parks": {}}
...
def run_rule_based_env_verify(config: VerifyConfig) -> None:
    ...
    try:
        print("env_verify_policy=rule_ev_charge bes=0 no_v2g")
        for episode in range(config.total_episodes):
            ...
            while not done:
                joint_action = _build_rule_based_joint_action(obs)
...
if __name__ == "__main__":
    run_rule_based_env_verify(VerifyConfig())
